...
const initState = {
  modals: [] as ModalType<any>[],
};

type Actions = {
  _private: {
    top: () => ModalType | undefined;
        ...
        set(({ modals: modalInfos }) => ({
          modals: modalInfos.concat({
            ...
            props: {
              ...options?.props,
              onSuccess: (value: any) =>
                get()._private.handlePromise(key, resolve, value),
              onClose: () =>
                get()._private.handlePromise(key, resolve, undefined),
            },
          }),
        }));
      });
